#!/bin/bash
set -x

DATADIR=/data
# Get current size of zabbix DB
size=`/bin/mysql -uroot -e "select table_schema,ROUND(sum(data_length + index_length) / 1024 / 1024) \
 from information_schema.tables WHERE table_schema = 'zabbix' GROUP BY table_schema;"|grep zabbix|awk '{print $NF}'`
echo $size

# First, check free space on /data
while true; do
    free_space=`df -k|grep '/data'|awk '{print $4}'`
    free_space=`expr $free_space / 1024`
    if [ $free_space -gt $size ];then
        break
    fi
done

# Second, remove dump's exept last 10
find /data -iname zabbix_dump_\*.sql|sort -r| tail -n +10|xargs rm -rf

# Last, do dump
date_stamp=`date +"%s"`
/bin/mysqldump zabbix >/data/zabbix_dump_$date_stamp.sql 